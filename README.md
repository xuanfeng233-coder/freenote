...
> Android 本地加密音乐解码器 — 纯 Kotlin，纯离线，无需服务器、无需联网。
...
把你**已经下载到本地**的加密音乐（网易云 `.ncm`、QQ音乐 `.qmc/.mflac`、酷狗 `.kgm`、酷我 `.kwm` 等）解码还原成普通的 FLAC / MP3 / OGG 文件。全程在手机本地完成，不上传任何文件、不连任何服务器。
...
1. 到本仓库的 **[Releases](../../releases)** 页面下载最新的 `app-release.apk`。
